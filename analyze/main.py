...
def main():
    ...
    for q in qs:
        Cn=Kd
        pars.append([N, imod,Ne, kar, lam, Kd, q, Cn, kards])
    ...
    for i in range(len(pars)):
        ...
        for kard in kards[::1]:
            if(i%1==0):
                pass
                filename = foldername + "/State_N%.0f_imod%.0f_Ne%.0f_kar%.0f_lam%.1f_Kd%.1f_q%.1f_Cn%.1f_kard%.1f.csv" % (N, imod,Ne, kar,lam,Kd,q,Cn,kard)
