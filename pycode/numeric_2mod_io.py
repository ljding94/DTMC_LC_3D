...
def F_compot_param(folder, m, alphas, gammas,  bn_phi, bn_z, R):
    ...
    with open(savename, "w") as f:
        f.write("alpha,gamma,intSS,intTT,intBB,intC\n")
        for i in range(len(intSS)):
            f.write("%f,%f,%f,%f,%f,%f\n" % (alpha_write[i],gamma_write[i],intSS[i],intTT[i],intBB[i],intC[i]))

from mpl_toolkits.axes_grid1 import make_axes_locatable
def F_compot_param_plot(filename,m):
    data = np.loadtxt(filename,delimiter=",",skiprows=1,unpack=True)
    alpha,gamma,intSS,intTT,intBB,intC = data
    cmap = cm.get_cmap("jet_r")
    plt.figure()
    fig, axs = plt.subplots(4,1,figsize=(4,3*4))
    ...
    im2 = axs[2].scatter(alpha,gamma,c=intBB,cmap=cmap)
    select = intBB==0
    ...
    im3 = axs[3].scatter(alpha,gamma,c=intC,cmap=cmap)
    select = intC==0
