...
def topo_data_get(Ne=2):
    if(Ne==2):
        foldername = "../data/Ne2/Feb28_2022"
...
def topo_change_plot(LineWidth, FontSize, LabelSize):
    ...
    axNe2.tick_params(which="both",direction="in", top="on", right="on",labelbottom=True, labelleft=True,labelsize=LabelSize)
    axNe2.set_ylabel(r"$|L_1-L_2|/(L_1+L_2)$", fontsize=FontSize)
    axNe2.set_ylim(0,1.0)
    axNe2.set_xlabel(r"$k_c$", fontsize=FontSize)
    axNe2.xaxis.set_major_locator(MultipleLocator(1))
    axNe2.xaxis.set_minor_locator(MultipleLocator(0.5))
    axNe2.yaxis.set_major_locator(MultipleLocator(0.1))
    axNe2.yaxis.set_minor_locator(MultipleLocator(0.05))
    ...
    for i in range(len(qcs)):
        ...
        ax_config_plot_xyz(axNe2c[i], fnamecs[i], "gray", LineWidth, rotxyz=rots[i],yshift=-20,mesh=1, bead=0,rod=1,d=1)
    ...
    axNe3.xaxis.set_major_locator(MultipleLocator(1))
    axNe3.xaxis.set_minor_locator(MultipleLocator(0.5))
    axNe3.yaxis.set_major_locator(MultipleLocator(0.1))
    axNe3.yaxis.set_minor_locator(MultipleLocator(0.05))
    ...
    qcs[0]=0.9
    for i in range(len(qcs)):
        axNe3c.append(fig.add_axes([qcs[i]/q[-1]-0.08, 0.12, 0.15, 0.3]))
        ax_config_plot_xyz(axNe3c[i], fnamecs[i], "gray", LineWidth, rotxyz=rots[i],mesh=1, bead=0,rod=0,d=1)
        ax_config_plot_xyz(axNe3c[i], fnamecs[i], "gray", LineWidth, rotxyz=rots[i],yshift=-20, mesh=1, bead=0,rod=1,d=1)
