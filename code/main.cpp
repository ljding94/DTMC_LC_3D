...
int main(int argc, char const *argv[])
{
    ...
    int id = 0; // id for this simulation, plan to run multiple simulation with same parameters
    ...
    std::string finfo = "N" + std::to_string(N) + "_imod" + std::string(argv[2]) + "_Ne" + std::string(argv[3]) + "_lf" + std::string(argv[4]) + "_kar" + std::string(argv[5]) + "_C0" + std::string(argv[6]) + "_karg" + std::string(argv[7]) + "_lam" + std::string(argv[8]) + "_Kd" + std::string(argv[9]) + "_q" + std::string(argv[10]) + "_Cn" + std::string(argv[11])+ "_id" + std::string(argv[12]); // + "_ku" + std::string(argv[12]);
    ...
    if (argc == 14)
    {
        ...
        membrane.O_MC_measure(100, 50, int(N / (delta_s * delta_s)) + 1, delta_s, delta_theta, delta_r, bin_num, folder, finfo, seq);
