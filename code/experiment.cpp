                           ...
                           std::string finfo)
{
    ...
    if (f.is_open())
    {
        ...
        f << "IdA,I2H,I2H2,IK,Tp2uu,Tuuc,Bond_num,Tun2,TRz\n";
        for (int i = 0; i < E_all.size(); i++)
        {
            ...
            f << IdA_all[i] << "," << I2H_all[i] << "," << I2H2_all[i] << "," << IK_all[i] << "," << Tp2uu_all[i] << "," << Tuuc_all[i] << "," << Bond_num_all[i] << "," << Tun2_all[i] << "," << TRz_all[i] << "\n";
        }
    }
    f.close();
